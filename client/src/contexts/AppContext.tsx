...
import { createAlert, createReport, createSensorReading, loadAlerts, loadLocations, loadReports, loadSensorReadings, markReportReviewed, seedLocationsIfEmpty, updateLocation, type AlertRecord, type FieldReport, type SensorReading } from "../lib/api";
import type { LocationRecord } from "../data/mockData";

type AppContextValue = { session: Session | null; user: User | null; locations: LocationRecord[]; reports: FieldReport[]; alerts: AlertRecord[]; readings: SensorReading[]; loading: boolean; error: string | null; refresh: () => Promise<void>; updateLocation: (id: string, patch: Partial<LocationRecord>) => Promise<void>; submitReport: (report: Omit<FieldReport, "id" | "created_at" | "reviewed">) => Promise<void>; reviewReport: (id: string, reviewed: boolean) => Promise<void>; addAlert: (alert: Omit<AlertRecord, "id" | "created_at" | "status">) => Promise<void>; addReading: (reading: Omit<SensorReading, "id" | "recorded_at">) => Promise<void>; };
